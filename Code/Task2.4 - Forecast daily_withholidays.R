...
#### ---- MODELLING ----
    ...
    ## ---- Prophet ----
      ...
      m <- add_country_holidays(train_prophet, country_name = 'FRA')
      m <- fit.prophet(m, df)
...
#### ---- Saving and plotting all the data ----
      ...
      write.csv2(correlations,'./Output/Daily_PostResample.csv', row.names = FALSE)
      
      saveRDS(metrics, './Output/Daily_metrics.RDS')
      write.csv2(metrics,'./Output/Daily_metrics.csv', row.names = FALSE)
      
      
      #### ---- MISCELLANEOUS ----

    #Testing things with Prophet
