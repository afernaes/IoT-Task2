...
#### ---- Monthly Time Series Creation and Analysis -----
  ...
  ##Create monthly time series
    ...
    #Autocorrelation: Look at the two notations
      autoplot(acf(MonthlyTS[,"GAP"], plot = FALSE))
...
#### ---- MODELLING -----
  ...
  ## ---- Arima ----
      ### ---- Automatic SARIMA ----
        MonthlyGAP_AR1<-auto.arima(TSTrainSet[,"GAP"], trace=T)
        MonthlyGAP_AR1
      ...
      ### ---- Manual ARIMA ----
        ...
        autoplot(acf(MonthlyTS[,"GAP"], plot = FALSE)) #q=2
        autoplot(pacf(MonthlyTS[,"GAP"], plot = FALSE)) #p=1
        
        
        MonthlyGAP_AR3<-arima(TSTrainSet[,"GAP"],
                              order=c(1,0,2), seasonal = list(order = c(1,1,0), period = 12))
  ...
  ## ---- Plotting all the models and Saving Data ----
        ...
        autoplot(TSTrainSet[,"GAP"], size=1.2) +
          
          autolayer(fc_MonthlyGAP_AR1$mean, series="SARIMA", size=1.2) +
         
          
          ggtitle("Forecasts for Monthly Energy Consumption") + theme(plot.title = element_text(hjust = 0.5))+
          xlab("Year") + 
          ylab("kWh") +
  ...
  #### ---- MISCELLANEOUS ----
        ...
        #Diff stuff
        nsdiffs(TSTrainSet[,"GAP"])
        
        sad<-diff(timeseries)
      
        
        nsdiffs(TSTrainSet[,"GAP"])
        ds.Monthly<- diff(MonthlyTS[,"GAP"],lag=12)
        
        autoplot(MonthlyTS[,"GAP"])
        autoplot(ds.Monthly)
        ...
        fit <- nnetar(TSTrainSet[,"GAP"])
        fcast <- forecast(fit, h=11)
        autoplot(fcast) + autolayer(TSTestSet[,"GAP"])
        accuracy(fcast)